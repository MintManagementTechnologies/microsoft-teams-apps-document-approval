import { EnvConfig } from '../../appConfig';
import {
    baseChatDeepLinkUrl, baseDocDeepLinkUrl, baseTaskDeepLinkUrl
} from '../../commonVariables';
import { docLibUrl } from '../../configVariables';
...
const taskModuleLinkHelper = (_action: string, _id: string) => {
   const appId = EnvConfig.getTeamsAppConfig()?.userTeamsAppId;
   const baseAppUrl = `${EnvConfig.getTeamsAppConfig()?.baseTeamsAppUrl}/index.html#/me/memo`;
   const appUrl = `${baseAppUrl}/${_action}/${_id}`;
   return appUrl;
   // const appUrl = encodeURIComponent(`${baseAppUrl}/${_action}/${_id}`);
   // return `${baseTaskDeepLinkUrl}/${appId}?url=${appUrl}&height=500&width=500&title=${i18n.t(`taskModule.title.${_action}`)}`;
}

const documentLinkHelper = (_docName: string) => {
   const docExtension = _docName.split(".")[1];
   return `${baseDocDeepLinkUrl}/${docExtension}/viewer/teams/${docLibUrl}~2F${_docName}`;
}

export const actionsManager = (personaType: 'requestor' | 'actor' | 'pa' | 'all', _notificationData: INotificationModel) => {
   const remindActor_notificationData:INotificationModel = {..._notificationData,
      recipients: _notificationData.recipients.filter((x:IRecipient) => x.personaType === 'actor')};

   const remindPA_notificationData:INotificationModel = {..._notificationData,
