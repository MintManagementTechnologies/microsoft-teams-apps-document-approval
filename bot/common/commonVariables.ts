export const baseTaskDeepLinkUrl = `https://teams.microsoft.com/l/task`;
export const baseChatDeepLinkUrl = `https://teams.microsoft.com/l/chat/0/0`;
export const baseDocDeepLinkUrl = `https://teams.microsoft.com/_#`;

export const defaultAvatar = 'data:image/jpg;base64,';

export const defaultErrors = {
   rtkApi: {
      ATTACHMENT: {
         GET_ATTACHMENT: {
            status: 'FETCH_ERROR',
            error: 'attachmentService - getAttachment'
         } as any,
         DELETE_ATTACHMENT: {
            status: 'DELETE_ERROR',
            error: 'attachmentService - deleteAttachment'
         } as any,
         UPLOAD_ATTACHMENT: {
            status: 'UPLOAD_ERROR',
            error: 'attachmentService - uploadAttachment'
         } as any,
      },
      NOTFOUND: {
         USERGOALS: {
            status: 'FETCH_ERROR',
            error: 'getAllUserGoals - UserGoalsNotFound'
         } as any,
         MYEMPLOYEESGOALS: {
            status: 'FETCH_ERROR',
            error: 'getAllMyEmployeesGoals - MyEmployeesGoalsNotFound'
         } as any
      },
      GRAPH: {
         SEARCH_USERS: {
            status: 'SEARCH_USERS',
            error: 'graphApiService - searchUsers'
         } as any,
      }
   }
}